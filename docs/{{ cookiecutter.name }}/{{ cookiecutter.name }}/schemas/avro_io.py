...
def validate(
    ...
    nodes = collections.deque(
        [
            ValidationNode(
                expected_schema, datum, getattr(expected_schema, "name", None),
            )
        ]
    )
...
class DatumReader:
    ...
    def writers_schema(
        self, writers_schema: avro.schema.Schema,
    ) -> None:
        self._writers_schema = writers_schema
    ...
    def readers_schema(
        self, readers_schema: avro.schema.Schema,
    ) -> None:
        self._readers_schema = readers_schema
    ...
    def read_data(
        ...
        readers_schema: avro.schema.Schema,
        decoder: "BinaryDecoder",
    ) -> object:
        # schema matching
        ...
        if writers_schema.type == "bytes":
            if logical_type == "decimal":
                ...
                if not (isinstance(precision, int) and precision > 0):
                    warnings.warn(
                        avro.errors.IgnoredLogicalType(
                            f"Invalid decimal precision {precision}. Must be a positive integer."
                        )
                    )
                ...
                if not (isinstance(scale, int) and scale >= 0):
                    warnings.warn(
                        avro.errors.IgnoredLogicalType(f"Invalid decimal scale {scale}. Must be a non-negative integer.")
                    )
                    return decoder.read_bytes()
    ...
    def read_fixed(
        self,
        writers_schema: avro.schema.FixedSchema,
        readers_schema: avro.schema.Schema,
        decoder: BinaryDecoder,
    ) -> bytes:
        """
    ...
    def read_enum(
        ...
        if index_of_symbol >= len(writers_schema.symbols):
            raise avro.errors.SchemaResolutionException(
                f"Can't access enum index {index_of_symbol} for enum with {len(writers_schema.symbols)} symbols",
                writers_schema,
                readers_schema,
            )
        ...
        if read_symbol not in readers_schema.symbols:
            raise avro.errors.SchemaResolutionException(
                f"Symbol {read_symbol} not present in Reader's Schema", writers_schema, readers_schema
            )
    ...
    def read_array(
        self,
        writers_schema: avro.schema.ArraySchema,
    ...
    def read_map(
        self,
        writers_schema: avro.schema.MapSchema,
    ...
    def read_union(
        self, writers_schema: avro.schema.UnionSchema, readers_schema: avro.schema.UnionSchema, decoder: BinaryDecoder
    ) -> object:
        """
        ...
        if index_of_schema >= len(writers_schema.schemas):
            raise avro.errors.SchemaResolutionException(
                f"Can't access branch index {index_of_schema} for union with {len(writers_schema.schemas)} branches",
                writers_schema,
                readers_schema,
            )
            ...
            for field_name, field in readers_fields_dict.items():
                if field_name not in writers_fields_dict:
                    if not field.has_default:
                        raise avro.errors.SchemaResolutionException(
                            f"No default value for field {field_name}", writers_schema, readers_schema
                        )
    ...
    def _read_default_value(self, field_schema: avro.schema.Schema, default_value: object) -> object:
        ...
        if isinstance(field_schema, avro.schema.MapSchema):
            if isinstance(default_value, Mapping):
                return {
                    key: self._read_default_value(field_schema.values, json_val) for key, json_val in default_value.items()
                }
            raise avro.errors.InvalidDefaultException(field_schema, default_value)
...
class DatumWriter:
    ...
    def write_record(
        self, writers_schema: avro.schema.RecordSchema, datum: Mapping[str, object], encoder: BinaryEncoder
    ) -> None:
        """
        A record is encoded by encoding the values of its fields
        in the order that they are declared. In other words, a record
