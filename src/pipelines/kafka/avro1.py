...
def _resolve_named_schema(schema, schema_registry_client, named_schemas=None):
    ...
    Args:
        schema: Schema to resolve named schemas for.
        schema_registry_client: SchemaRegistryClient to use for retrieval.
        named_schemas: Dict of named schemas resolved recursively.
...
def _adjust_decimal_precision(value, schema):
    """
    Adjusts the precision and scale of a Decimal value to match the Avro schema.

    Args:
        value (Decimal): The decimal value to adjust.
        schema (dict): The Avro schema.

    Returns:
        Decimal: The adjusted decimal value.
    """
    if 'logicalType' in schema and schema['logicalType'] == 'decimal':
...
class AvroSerializer(Serializer):
    ...
    Note:
        Tuple notation can be used to determine which branch of an
        ambiguous union to take.

    """

    __slots__ = [
        '_hash', '_auto_register', '_normalize_schemas', '_use_latest_version',
        '_known_subjects', '_parsed_schema', '_registry', '_schema', '_schema_id',
        '_schema_name', '_subject_name_func', '_to_dict', '_named_schemas'
    ]
    ...
    def __init__(self, schema_registry_client, schema_str, to_dict=None, conf=None):
        ...
        Args:
            ...
            to_dict (callable, optional):
                Callable(object, SerializationContext) -> Dict[str, Any].
                Converts object to a dict.

            conf (dict, optional): AvroSerializer configuration.

        Raises:
            ValueError:
                If any error occurs serializing obj.

            SchemaRegistryError:
                If there was an error registering the schema with
                Schema Registry, or auto.register.schemas is false and the schema was not registered.

            TypeError:
                If you must pass either schema string or schema object.
        ...
        if len(conf_copy) > 0:
            raise ValueError(f"Unrecognized properties: {', '.join(conf_copy.keys())}")
    ...
    def __call__(self, obj, ctx):
        """Serializes an object to Avro binary format, prepending it with Schema Registry framing.
        ...
        # Adjust decimal precision and scale
        for field in self._parsed_schema.get('fields', []):
            if isinstance(value.get(field['name']), Decimal):
                value[field['name']] = _adjust_decimal_precision(value[field['name']], field['type'])

        with _ContextStringIO() as fo:
            # Write the magic byte and schema ID in
...
class AvroDeserializer(Deserializer):
    ...
    __slots__ = [
        '_reader_schema', '_registry', '_from_dict', '_writer_schemas', '_return_record_name',
        '_schema', '_named_schemas'
    ]
    ...
    def __init__(
        ...
        """Initialize the AvroDeserializer.
        ...
        Raises:
            ValueError:
                If any error occurs deserializing data.
