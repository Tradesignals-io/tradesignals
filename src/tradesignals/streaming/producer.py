...
from tradesignals.streaming.kafka.librdkafka import KafkaConfig
from tradesignals.common.types.core import DecimalT
...
class AvroProducer(ABC):
    ...
    def decimal_to_bytes(self, value: Union[Decimal, DecimalT], precision: int = 26, scale: int = 4) -> bytes:
        """
    ...
    def get_latest_schema(self) -> Union[str, None]:
        """
        ...
        try:
            ...
            logger.info("Latest schema for topic: %s", json.dumps(response.json(), sort_keys=True, indent=2))
    ...
    async def produce_message(self, record: Dict[str, Any]) -> None:
        ...
        try:
            ...
            value = self.set_message_value(record, msg_id=msg_id)
            encoded_value = self.serializer.encode_record_with_schema(self.topic, self.avro_schema, value, is_key=False)
            headers = self.set_message_headers(record, msg_id=msg_id) or {}
            encoded_headers = [(key, value.encode()) for key, value in headers.items()]
            key = self.set_message_key(record, msg_id=msg_id)
            if key is None:
                key = msg_id
            ts = self.set_message_timestamp(record)
            if ts is None:
